/**
 * Import Notes Management JavaScript
 * Xử lý các chức năng liên quan đến quản lý phiếu nhập
 */
...
function addImportNote() {
    window.location.href = 'import-note-create.html';
}

// Edit import note
function editImportNote(noteId) {
    window.location.href = `import-note-detail.html?id=${noteId}`;
}

// Delete import note - Show modal
let currentDeleteNoteId = null;

function deleteImportNote(noteId, noteCode) {
    currentDeleteNoteId = noteId;
    const modal = document.getElementById('deleteModal');
    const noteCodeSpan = document.getElementById('deleteImportNoteCode');
...
function confirmDelete() {
    if (currentDeleteNoteId) {
        ...
        console.log('Deleting import note:', currentDeleteNoteId);
...
document.addEventListener('DOMContentLoaded', function() {
    // Auto-generate import note code if on create page
    if (window.location.pathname.includes('import-note-create.html')) {
        const codeInput = document.getElementById('importNoteCode');
        const batchInput = document.getElementById('importBatch');
        ...
        if (codeInput && !codeInput.value) {
            ...
            codeInput.value = `PN_${dateStr}_${random}`;
        }
        ...
        if (batchInput && !batchInput.value) {
            ...
            const dateStr = now.toISOString().slice(0, 10).replace(/-/g, '');
            const random = Math.floor(Math.random() * 1000);
            batchInput.value = `LN_${dateStr}_${random}`;
        }
